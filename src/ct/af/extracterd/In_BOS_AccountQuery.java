...
public class In_BOS_AccountQuery {
	...
	public AFSubInstance extractRawData(AbstractAF abstractAF,EC02Instance ec02Instance,AFSubInstance afSubIns,EquinoxRawData eqxRawData) {
		...
		if(eqxRawData.getRawDataMessage() != null
			...
			In_BOS_SubscriberAccountEnquiryParser accountQueryParser = In_BOS_SubscriberAccountEnquiryParser.getInstance();
			Param_BOS_SubscriberAccountEnquiry accountQueryParameter = accountQueryParser.doParser(eqxRawData);
			
			isValid = accountQueryParameter.isValid();
			
			if(isValid) {
				
				afSubIns.setSubResponseParam(accountQueryParameter);
				statIn = EStats.INGATEWAY_RECEIVE_BOS_ACCOUNTQUERY_RESPONSE;
				resultCode = accountQueryParameter.getRespResultCode();
		...
		afSubIns.setSubCurrentState(ESubState.BOS_AccountQuery.toString());
		afSubIns.setSubControlState(ESubState.BOS_AccountQuery.toString());
