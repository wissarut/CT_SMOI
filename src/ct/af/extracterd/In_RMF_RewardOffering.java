...
public class In_RMF_RewardOffering {
	...
	public AFSubInstance extractRawData(AbstractAF abstractAF,EC02Instance ec02Instance,AFSubInstance afSubIns,EquinoxRawData eqxRawData) {
		...
		if(eqxRawData.getRawDataMessage() != null
			...
			if(isValid) {
				...
				statIn = EStats.INGATEWAY_RECEIVE_RMF_REWARDOFFERING_RESPONSE;
				resultCode = rewardMonitoringParameter.getRespResultCode();
				
			} else {
				
				statIn = EStats.INGATEWAY_RECEIVE_RMF_BAD_REWARDOFFERING_RESPONSE;
				resultCode = rewardMonitoringParameter.getRespResultCode();
			...
			if(rawDataRet.equals(ERet.RET4.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_RMF_REWARDOFFERING_TIMEOUT;
				resultCode = EResultCode.REret4;
				isRetNormal = false;
			} else if(rawDataRet.equals(ERet.RET2.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_RMF_REWARDOFFERING_REJECT;
				resultCode = EResultCode.REret2;
				isRetNormal = false;
			} else if(!rawDataECode.equals("200")) {
				statIn = EStats.INGATEWAY_RECEIVE_RMF_REWARDOFFERING_ERROR;
		...
		afSubIns.setSubCurrentState(ESubState.RMF_RewardOffering.toString());
