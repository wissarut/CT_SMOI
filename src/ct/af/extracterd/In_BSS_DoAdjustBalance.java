...
import ct.af.message.incoming.parser.In_BSS_DoAdjustBalanceParser;
import ec02.af.abstracts.AbstractAF;
import ec02.af.data.EquinoxRawData;

public class In_BSS_DoAdjustBalance {
	...
	public AFSubInstance extractRawData(AbstractAF abstractAF,EC02Instance ec02Instance,AFSubInstance afSubIns,EquinoxRawData eqxRawData) {
		...
		if(eqxRawData.getRawDataMessage() != null
			...
			In_BSS_DoAdjustBalanceParser doAdjustBalanceParser = In_BSS_DoAdjustBalanceParser.getInstance();
			Param_BSS_DoAdjustBalance doAdjustBalanceParameter = doAdjustBalanceParser.doParser(eqxRawData);
			
			isValid = doAdjustBalanceParameter.isValid();
			
			if(isValid) {
				
				afSubIns.setSubResponseParam(doAdjustBalanceParameter);
				statIn = EStats.INGATEWAY_RECEIVE_BSS_DOADJUSTBALANCE_RESPONSE_SUCCESS;
				resultCode = doAdjustBalanceParameter.getRespResultCode();
				
			} else {
				
				statIn = EStats.INGATEWAY_RECEIVE_BSS_BAD_DOADJUSTBALANCE_RESPONSE;
				resultCode = doAdjustBalanceParameter.getRespResultCode();
			...
			if(rawDataRet.equals(ERet.RET4.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_BSS_DOADJUSTBALANCE_REQUEST_TIMEOUT;
				resultCode = EResultCode.REret4;
				isRetNormal = false;
			} else if(rawDataRet.equals(ERet.RET2.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_BSS_DOADJUSTBALANCE_REQUEST_REJECT;
				resultCode = EResultCode.REret2;
				isRetNormal = false;
			} else if(!rawDataECode.equals("200")) {
				statIn = EStats.INGATEWAY_RECEIVE_BSS_DOADJUSTBALANCE_REQUEST_ERROR;
		...
		afSubIns.setSubCurrentState(ESubState.BSS_DoAdjustBalance.toString());
