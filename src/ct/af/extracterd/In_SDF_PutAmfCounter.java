...
import ct.af.message.incoming.parser.In_SDF_PutAmfCounterParser;
import ec02.af.abstracts.AbstractAF;
import ec02.af.data.EquinoxRawData;

public class In_SDF_PutAmfCounter {
	...
	public AFSubInstance extractRawData(AbstractAF abstractAF,EC02Instance ec02Instance,AFSubInstance afSubIns,EquinoxRawData eqxRawData) {
		...
		if(eqxRawData.getRawDataMessage() != null
			...
			In_SDF_PutAmfCounterParser sdfPutAmfCounterParser = In_SDF_PutAmfCounterParser.getInstance();
			Param_SDF_PutAmfCounter sdfPutAmfCounterParameter = sdfPutAmfCounterParser.doParser(eqxRawData);
			
			isValid = sdfPutAmfCounterParameter.isValid();
			
			if(isValid) {

				afSubIns.setSubResponseParam(sdfPutAmfCounterParameter);
				statIn = EStats.INGATEWAY_RECEIVE_SDF_PUT_AMFCOUNTER_RESPONSE;
				resultCode = sdfPutAmfCounterParameter.getRespResultCode();
				
			} else {
				
				if(sdfPutAmfCounterParameter.getRespResultCode().equals(EResultCode.RE50000)) {
					
					statIn = EStats.INGATEWAY_RECEIVE_SDF_PUT_AMFCOUNTER_ERROR;
					resultCode = sdfPutAmfCounterParameter.getRespResultCode();
					
				} else {
					
					statIn = EStats.INGATEWAY_RECEIVE_SDF_BAD_PUT_AMFCOUNTER_RESPONSE;
					resultCode = sdfPutAmfCounterParameter.getRespResultCode();
				}
			...
			if(rawDataRet.equals(ERet.RET4.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_SDF_PUT_AMFCOUNTER_TIMEOUT;
				resultCode = EResultCode.REret4;
				isRetNormal = false;
			} else if(rawDataRet.equals(ERet.RET2.getRet())) {
				statIn = EStats.INGATEWAY_RECEIVE_SDF_PUT_AMFCOUNTER_REJECT;
		...
		afSubIns.setSubCurrentState(ESubState.SDF_PutAmfCounter.toString());
		afSubIns.setSubControlState(ESubState.SDF_PutAmfCounter.toString());
