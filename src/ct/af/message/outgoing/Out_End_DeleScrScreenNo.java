...
public class Out_End_DeleScrScreenNo {
	...
	public EquinoxRawData messageBuilder(AbstractAF abstractAF,EC02Instance ec02Instance,AFSubInstance afSubIns) {
		...
		if(finaleCode.getFinalResultCode().equals("000")) {
			...
			if(ec02Instance.getAFInstance().getMainProfile().equals(EFunctionGroup.INS.toString())) {
				...
				strbuilder.append("<screentype>"+afSubIns.getClientData().getScreentype()+"</screentype>")
						.append("<totalnum>"+datArr.size()+"</totalnum>")
						.append("<succnum>"+datArr.size()+"</succnum>")
						.append("<successArray>");
				
				for(String msisdn:datArr) {
					strbuilder.append("<msisdn>"+msisdn+"</msisdn>");
				}
				
				strbuilder.append("</successArray>")
						.append("<existnum></existnum>")
						.append("<existArray>")
						.append("</existArray>");
